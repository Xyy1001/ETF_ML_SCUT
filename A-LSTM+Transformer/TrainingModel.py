...
folder_path = "/root/ETF/data/raw_data"
...
if __name__ == "__main__":
    # 加载股票数据
    dataframes = load_stock_data_csv(folder_path)
    ...
    for i in range(len(dataframes)):
        ...
        # 以股票文件名作为模型保存路径的一部分
        path = f"ETF/model/{data.name}.pth"

        # 训练并保存模型
        train_and_save_model(train_loader, num_epochs=100, learning_rate=0.005, model_path=path)