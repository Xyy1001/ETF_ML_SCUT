...
from gnn_model import GNNModel, frobenius_norm_loss
from datasets import get_dataloaders
from plot_utils import plot_training_curves, plot_corr_matrices
...
def evaluate(config_path: str = 'configs/variables.yaml'):
    ...
    # 在测试集上评估
    all_losses = []
    all_predicted_matrices = []  # 存储所有预测的相关系数矩阵
    all_true_matrices = []       # 存储所有真实的相关系数矩阵
    example_true = None
    example_pred = None
    with torch.no_grad():
    ...
    loss_val = float(np.mean(all_losses))
    ...
    predicted_matrices_path = os.path.join(cfg['outputs']['results_dir'], 'predicted_correlation_matrices.npy')
    true_matrices_path = os.path.join(cfg['outputs']['results_dir'], 'true_correlation_matrices.npy')
    
    np.save(predicted_matrices_path, all_predicted_matrices)
    np.save(true_matrices_path, all_true_matrices)
    
    print(f"保存了 {all_predicted_matrices.shape[0]} 个预测相关系数矩阵到: {predicted_matrices_path}")
    print(f"保存了 {all_true_matrices.shape[0]} 个真实相关系数矩阵到: {true_matrices_path}")
    print(f"矩阵形状: {all_predicted_matrices.shape}")
    
    with open(cfg['outputs']['mse_score'], 'w', encoding='utf-8') as f:
        f.write(f"Frobenius Norm Loss on test set: {loss_val:.8f}\n")
    ...
    if example_true is not None and example_pred is not None:
        plot_corr_matrices(example_true, example_pred, cfg['outputs']['results_dir'], step=0)

    with open(cfg['outputs']['evaluation_report'], 'w', encoding='utf-8') as f:
        f.write(f"Test Frobenius Norm Loss: {loss_val:.8f}\n")
