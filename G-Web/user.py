...
def retry_once_on_db_error(func: Callable) -> Callable:
    """Decorator: retry DB cursor operation exactly once after reconnect on transient failures."""
    ...
    def wrapper(self, *args, **kwargs):
        try:
            ...
            if not self.manager.is_retryable_db_error(exc):
                raise

            if not self.manager.reconnect_for_retry():
                raise

            # Rebind to newly created cursor proxy after reconnect.
            rebound = self.manager.cursor
            if rebound is None:
                raise
            return func(rebound, *args, **kwargs)

    return wrapper


class RetryCursorProxy:
    """Transparent cursor proxy: wraps execute/executemany with one reconnect retry."""

    def __init__(self, manager: 'UserManager', raw_cursor):
        self.manager = manager
        self._cursor = raw_cursor

    @retry_once_on_db_error
...
class UserManager:
    ...
    def _create_retry_cursor(self):
        """创建带重试功能的cursor，失败时返回None而不抛异常"""
        if self.conn is None:
            return None
        try:
            raw_cursor = self.conn.cursor()
            return RetryCursorProxy(self, raw_cursor)
        except Exception as e:
            print(f"创建cursor失败: {e}")
            return None

    def is_retryable_db_error(self, exc: Exception) -> bool:
        msg = str(exc).lower()
        if isinstance(exc, (pymysql.OperationalError, pymysql.InterfaceError)):
            return True
        keywords = [
            'read of closed file',
    ...
    def reconnect_for_retry(self) -> bool:
        """Best-effort reconnect used by retry proxy."""
        try:
            self.disconnect()
            return self.connect()
        except Exception:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.cursor = self._create_retry_cursor()
