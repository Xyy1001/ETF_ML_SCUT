'''
这是数据预处理模块，负责从原始XLSX文件中读取股票数据，提取特征和目标变量，
将数据切分为训练集和测试集，构造滑动窗口样本，并对特征进行标准化处理，最后保存处理后的数据以供模型训练使用。
'''
import os
import glob
import pickle
...
def _build_samples(features_list: List[np.ndarray], log_returns_list: List[np.ndarray],
    ...
    Returns:
        ...
        edge_features: [n_samples, n_stocks, n_stocks] 边特征（协方差矩阵）
        targets: [n_samples, n_stocks, n_stocks] 目标（未来10天协方差矩阵）
    """
    ...
    print(f"构造样本：{n_samples}个样本，每个样本使用{window_size}天数据")
    
    # 初始化输出数组
    node_features = np.zeros((n_samples, n_stocks, 36, lookback), dtype=np.float64)
    edge_features = np.zeros((n_samples, n_stocks, n_stocks), dtype=np.float64)
    targets = np.zeros((n_samples, n_stocks, n_stocks), dtype=np.float64)
    ...
    for i in range(n_samples):
        ...
        # 1. 构造节点特征：每只股票前100天的36维特征
        for stock_idx in range(n_stocks):
            # 取前100天的特征数据 [100, 36] -> 转置为 [36, 100]
            stock_features = features_list[stock_idx][start_idx:lookback_end]  # [100, 36]
            node_features[i, stock_idx] = stock_features.T  # [36, 100]
        
        # 2. 构造边特征：前100天各股票对数收益率的协方差矩阵
        ...
        # 将对数收益率乘以100
        lookback_returns = lookback_returns * 100
        
        # 计算协方差矩阵
        try:
            if np.std(lookback_returns) < 1e-8:
                cov_matrix = np.eye(n_stocks, dtype=np.float64)
            else:
                cov_matrix = np.cov(lookback_returns)
                cov_matrix = np.clip(cov_matrix, -10.0, 10.0)
                cov_matrix += np.eye(n_stocks) * 0.01  # 正则化
        except:
            cov_matrix = np.eye(n_stocks, dtype=np.float64)
        
        edge_features[i] = cov_matrix.astype(np.float64)
        
        # 3. 构造目标：后10天各股票对数收益率的协方差矩阵
        ...
        # 将对数收益率乘以100
        future_returns = future_returns * 100
        
        # 计算未来10天的协方差矩阵
        try:
            if np.std(future_returns) < 1e-8:
                target_cov = np.eye(n_stocks, dtype=np.float64)
            else:
                target_cov = np.cov(future_returns)
                target_cov = np.clip(target_cov, -10.0, 10.0)
                target_cov += np.eye(n_stocks) * 0.01  # 正则化
        except:
            target_cov = np.eye(n_stocks, dtype=np.float64)
        
        targets[i] = target_cov.astype(np.float64)
...
def preprocess(config_path: str = "E:/Machine_Learning_ETF/GNN/configs/variables.yaml") -> None:
