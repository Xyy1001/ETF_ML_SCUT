...
def resolve_env_path(env_path: str = ".env") -> Path:
    raw = Path(env_path)
...
resolved_env = resolve_env_path(".env")
loaded = dotenv.load_dotenv(dotenv_path=resolved_env)
if loaded:
    print(f"Loaded .env: {resolved_env}")
else:
    print(f"Warning: .env not found at {resolved_env}, using current environment variables")

API_KEY = os.getenv("TUSHARE_TOKEN", "").strip()
if not API_KEY:
...
# 设置数据分割时间
start_date = (os.getenv("start_date", "20100101") or "20100101").strip()
end_date = (os.getenv("end_date", "") or "").strip()
if not end_date:
    end_date = pd.Timestamp.today().strftime("%Y%m%d")

atr_n = int((os.getenv("ATR_N", "14") or "14").strip())
rsi_n = int((os.getenv("RSI_N", "14") or "14").strip())

# 用户输入的股票代码列表
print("股票列表（前30条）:")
stock_list = pro.stock_basic(exchange="", list_status="L", fields="ts_code,name,area,industry")
print(stock_list.head(30))
print("参考输入格式：000001.SZ,600519.SH")
user_codes = [code.strip().upper() for code in input("请输入股票代码，多个代码用逗号分隔: ").split(",") if code.strip()]

# 数据列表
data_list = []


def fetch_stock_daily(ts_code: str) -> pd.DataFrame:
    df = pro.daily(
        ts_code=ts_code,
...
# 获取多个股票的日线数据
for stock in user_codes:
    data = fetch_stock_daily(stock)
    if data.empty:
        print(f"{stock} 未获取到有效数据，已跳过")
        continue
    data_list.append(data)
    print(f"Fetched data for {stock}: {data.shape}")

print(f"Total valid symbols: {len(data_list)}")


def sum_daily_data_reduce(frames):
    """按 trade_date 对齐后，对数值字段做逐日加总。"""
    if not frames:
        return pd.DataFrame(columns=["trade_date"] + NUMERIC_COLUMNS)

    concat_df = pd.concat(frames, ignore_index=True)
    grouped = concat_df.groupby("trade_date", as_index=False)[NUMERIC_COLUMNS].sum(min_count=1)
    grouped = grouped.sort_values("trade_date").reset_index(drop=True)
    grouped["ts_code"] = "TOTAL"

    ordered_cols = ["ts_code", "trade_date"] + NUMERIC_COLUMNS
    return grouped[ordered_cols]


# 创建组合表：每天将股票池所有标的数值字段加总
...
def calculate_atr_indicators(daily_total: pd.DataFrame, n: int = 14):
    """
    ...
    for i in range(len(daily_total)):
        if i == 0:
            # 与 daily_update.py 保持一致：首日 TR = high - low
...
def calculate_rsi_indicators(daily_total: pd.DataFrame, n: int = 14):
    """
    计算相对强弱指数(RSI)指标
...
def forward_fill_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """与数据库脚本保持一致：指标列使用前向填充。"""
    existing_indicator_cols = [col for col in INDICATOR_COLUMNS if col in df.columns]
    if existing_indicator_cols:
        df[existing_indicator_cols] = df[existing_indicator_cols].ffill()
    return df


"""保存成交量变动率处理数据"""
def save_data(data: pd.DataFrame, folder_path: str):
...
output_folder = input("请输入保存路径（默认当前路径）: ").strip() or "Transformer+LSTM/Advanced/data"
...
for data in data_list:
    ...
    calculate_atr_indicators(data, n=atr_n)
    calculate_rsi_indicators(data, n=rsi_n)
    calculate_macd_indicators(data)
    calculate_volume_change_rate(data)
    forward_fill_indicators(data)
    save_data(data, output_folder)

# 保存多资产组合数据
calculate_mtm_indicators(daily_total)
calculate_ma_indicators(daily_total)
calculate_ema_indicators(daily_total)
calculate_atr_indicators(daily_total, n=atr_n)
calculate_rsi_indicators(daily_total, n=rsi_n)
calculate_macd_indicators(daily_total)
calculate_volume_change_rate(daily_total)
forward_fill_indicators(daily_total)
save_data(daily_total, output_folder)