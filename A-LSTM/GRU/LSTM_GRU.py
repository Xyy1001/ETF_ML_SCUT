...
3. 保存训练曲线
4. 保存模型
python GRU/LSTM_GRU.py --data-file data/000001.SZ.csv --model-dir model_gru --plot-dir plots_gru
"""
import argparse
import math
import os
...
DEFAULT_CSV_PATH = "/root/ETF/data/raw_data/000001.SZ.csv"
...
def load_single_stock_csv(csv_path):
    if not csv_path:
        print("未提供CSV文件路径")
        return None
    ...
    if not os.path.isfile(csv_path):
        print(f"路径不是文件，请检查路径: {csv_path}")
        return None
    if not csv_path.endswith(".csv"):
        print(f"文件不是CSV，请检查路径: {csv_path}")
        return None

    df = pd.read_csv(csv_path)
    df = fill_missing_values(df)
    df.name = re.sub(r"\.csv$", "", os.path.basename(csv_path))
    print(f"读取到数据文件: {csv_path}")
    return df
...
def parse_args():
    parser = argparse.ArgumentParser(description="训练 LSTM + GRU 股票预测模型（单只股票）")
    parser.add_argument(
        "--data-file",
        type=str,
        default=None,
        help="CSV文件路径（为空则提示输入）",
    )
    parser.add_argument(
        "--input-window",
        type=int,
...
def resolve_data_file(cli_path):
    if cli_path:
        return cli_path
    user_input = input(f"请输入CSV文件路径 (默认: {DEFAULT_CSV_PATH}): ").strip()
    return user_input if user_input else DEFAULT_CSV_PATH
...
if __name__ == "__main__":
    ...
    args = parse_args()
    csv_path = resolve_data_file(args.data_file)

    data = load_single_stock_csv(csv_path)
    if data is None:
        print("数据加载失败，程序结束。")
        raise SystemExit(1)
